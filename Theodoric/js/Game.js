...
Theodoric.Game.prototype = {
    ...
    update: function () {
        ...
        if (this.game.time.now > this.player.lastCombatTime + 5000) { // 5 seconds of no combat
            if (this.game.time.now % 1000 === 0) { // Heal every second
                if (this.player.health < this.player.vitality) { // Only heal if health is below max
                    this.player.health += 1;
                    if (this.player.health > this.player.vitality) {
                        this.player.health = this.player.vitality; // Cap health at max
                    }
                    this.notification = 'Regenerating health...'; // Only show when healing happens
                }
            }
        }
    },
    ...
    hit: function (target, attacker) {
        target.lastCombatTime = this.game.time.now; // Update combat time
    ...
    generatePlayer: function () {
        ...
        player.lastCombatTime = this.game.time.now; // Initialize combat time
    ...
    generateSkeleton: function (enemy) {
        ...
        return this.setStats(enemy, 'Skeleton', 40, 20, 5, 5, 6);
    },
    ...
    generateSpider: function (enemy) {
        ...
        return this.setStats(enemy, 'Spider', 20, 20, 12, 4, 10);
    },
